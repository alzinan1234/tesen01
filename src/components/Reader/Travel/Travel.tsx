...
// ── Main Travel Component (static category) ─────────────────────
const Travel = () => {
  const router = useRouter();
  const category = "travel"; // fixed category
  ...
  const handleShare = async (story: Story) => {
    const url = `${window.location.origin}/reader/travel/${story._id}`;
    if (navigator.share) {
  ...
  const handleStoryClick = (story: Story, e: React.MouseEvent) => {
    ...
    if (story.isPremium) {
      ...
      router.push(`/reader/travel/${story._id}`);
    }
  };
  ...
  return (
    <main className="bg-white min-h-screen pt-28 md:pt-64 pb-10">
      ...
      <div className="max-w-4xl mx-auto text-center mb-16 px-4">
        <h1 className="text-6xl font-sans text-gray-900 mb-4 font-extrabold tracking-wide capitalize">Travel</h1>
