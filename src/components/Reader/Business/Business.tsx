...
const Business = () => {
  ...
  const [articles, setArticles] = useState(businessArticles.map(a => ({
    ...a,
    isLiked: false,
  ...
  return (
    <main className="bg-white min-h-screen pt-28 md:pt-64 pb-10">
      <div className="max-w-4xl mx-auto text-center mb-16 px-4">
        <h1 className="text-6xl font-sans text-gray-900 mb-4 font-extrabold tracking-wide">Business</h1>
        <p className="text-black font-serif text-lg tracking-wide">Markets, Innovation, and the Global Economy.</p>
          ...
          <div key={article.id} className="flex flex-col border-b border-gray-50 pb-12 group">
            <div className="flex flex-col md:flex-row gap-10 items-start">
              <div className="flex-1 order-2 md:order-1">
                ...
                <Link href={`/reader/business/${article.id}`}>
              ...
              <Link href={`/reader/business/${article.id}`} className="w-full md:w-[320px] h-[200px] rounded-2xl overflow-hidden order-1 md:order-2 shadow-sm">
            ...
            {article.showCommentBox && (
              <div className="mt-6 bg-[#F2F2F2] rounded-2xl p-6 transition-all">
                ...
                <div className="flex justify-start items-center gap-6 mt-4">
                  ...
                  <button className="px-8 py-2.5 bg-[#D1D5DB] text-black rounded-full font-serif text-sm hover:bg-gray-300 transition-colors tracking-wide">Submit</button>
                </div>
              </div>
            )}
          </div>
