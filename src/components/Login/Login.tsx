...
import { login, socialLogin } from "../apiClient";
...
const Toast: React.FC<ToastProps> = ({ message, type, onClose }) => {
  ...
  return (
    <div
      className={`fixed top-5 right-5 z-[999] flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl font-serif text-sm text-white max-w-sm ${bg[type]}`}
...
// ── Extract a clean error message from Axios errors ───────────
...
const LoginPage: React.FC = () => {
  ...
  const showToast = useCallback(
    (message: string, type: ToastType = "info"): void => setToast({ message, type }),
    []
  );
  ...
  // The role selected in the UI IS the role sent to the API.
  // If the user picks "Writer" but types a Reader-only email,
  // the backend will return 401/403 and we show that error.
  // No client-side cross-role guessing needed.

  const onSubmit = async (values: LoginFormValues): Promise<void> => {
    const roleUsed: Role = activeRole; // capture — never stale inside timeout
    ...
    try {
      ...
      // The backend returns a specific message when the role is wrong,
      // e.g. "User not found" or "Invalid credentials for this role".
      // We display that message directly so the user knows to switch roles.
  ...
  // ── Social Login ──────────────────────────────────────────
  // Replace demoUser block with real Firebase / Google One Tap / Apple Sign In.

  const handleSocialLogin = async (provider: "google" | "apple"): Promise<void> => {
    const roleUsed: Role = activeRole;
    setSocialLoading(provider);

    try {
      // ── Replace with real OAuth SDK ──────────────────────
      const demoUser = { name: "Demo User", email: "demo@gmail.com", photo: "" };
      // ────────────────────────────────────────────────────
      ...
      if (res.success) {
        showToast(res.message || "Social login successful!", "success");
        setTimeout(() => router.push(getRedirectPath(roleUsed)), 1000);
      }
    } catch (err: unknown) {
      const message = extractErrorMessage(err, "Social login failed. Please try again.");
      showToast(message, "error");
    } finally {
      ...
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-[592px] min-h-[775px] flex flex-col py-12 px-6 sm:px-12 bg-white">
          ...
          <div className="w-full max-w-[400px]">
            ...
            <div className="flex justify-center gap-6 mb-8">
              <button
                ...
                className="w-[44px] h-[44px] flex items-center justify-center rounded-full bg-black hover:bg-gray-900 transition-all active:scale-95 shadow-md disabled:opacity-60"
              >
              ...
              >
                {socialLoading === "apple"
                  ? <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  : <img src="https://upload.wikimedia.org/wikipedia/commons/f/fa/Apple_logo_black.svg" className="w-6 h-6 brightness-0 invert" alt="Apple" />}
              </button>
            </div>
            ...
            <p className="text-center text-sm text-[#2D2D2D]">
              ...
              <Link href="/signup" className="text-[#3448D6] font-serif font-bold text-[16px] ml-1">
                Sign Up
              </Link>
            </p>
          </div>
          ...
          <footer className="mt-auto pt-16 w-full flex flex-col sm:flex-row justify-between gap-2 text-[14px] text-[#2D2D2D] tracking-widest font-sans">
            ...
            <Link href="#" className="text-[#3448D6] normal-case font-bold">
              Terms &amp; Conditions
            </Link>
          </footer>
        </div>
