...
import { cultureArticles as politicsArticles } from '@/components/data/cultureData';

const PoliticsStory = ({ params }: { params: { id: string } }) => {
  const story = politicsArticles.find(s => s.id === params.id) || politicsArticles[0];
    ...
    <main className="bg-white min-h-screen">
      ...
      <section className="relative w-full min-h-[80vh] md:h-screen flex flex-col md:flex-row bg-black pt-20 md:pt-24 overflow-hidden">
        <div className="w-full md:w-1/2 flex items-center justify-center bg-black text-white px-6 py-12 lg:px-20 z-10">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="max-w-[540px] text-center flex flex-col items-center">
            ...
            <motion.a href="#content" whileHover={{ scale: 1.05 }} className="flex  font-sans items-center gap-2 text-[#3448D6] font-bold text-lg mb-12">
              Full Report <ArrowUpRight size={20} />
            </motion.a>
            <div className="flex items-center gap-10 pt-8 font-sans border-t border-white/10 w-full justify-center">
        ...
        <div className="w-full md:w-1/2 h-[300px] md:h-full relative">
          <img src={story.image} className="w-full h-full object-cover" alt="Politics Hero" />
        ...
        <div className="flex-1">
          <article className="prose prose-lg text-black leading-loose">
            <p className="mb-6 font-extrabold text-2xl font-sans text-black">
              The shift in legislative power this season marks a turning point for domestic policy and international relations...
            </p>
            <p className="mb-6 font-serif text-lg text-black">
              For decades, the balance of power has relied on institutional norms that are now being tested. Our analysis explores how grassroots movements are bypassing traditional lobbying to influence the highest levels of government directly.
            </p>
            <p className="mb-8 font-serif border-l-4 border-black pl-6 py-2 italic text-xl text-black">
              "True political power does not reside in the halls of government, but in the collective will of the people who demand change."
            </p>
            <p className="mb-6 font-serif text-lg text-black">
              As we look toward the upcoming election cycle, the data suggests that economic policy remains the primary driver for suburban voters. However, climate legislation has emerged as a critical secondary factor for the younger demographic, creating a complex map for strategists.
            </p>
          </article>
          ...
          {/* Comment Section matching your UI requirements */}
          <AnimatePresence>
            {showCommentBox && (
